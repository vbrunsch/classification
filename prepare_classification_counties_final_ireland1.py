...
tim = list(e_dataframe0.columns)[n:]
tim.pop(0)
...
for name in counties:
    ...
    print(len(e_dataframe1[name].to_list()))
    values0=[x for x in e_dataframe1[name][n:]]#.replace(len(e_dataframe1[name])-15,e_dataframe1[name][len(e_dataframe1[name])-16])
    values=[]
    indd=0
    for en in values0:
        if indd==121:
    ...
    x = e_dataframe1[e_dataframe1.columns[0]][n:]
    y1 = interpolate(y)    
    x2 = x[9:]
    tim2 = tim[4 : -5]
